...
const main = function (input) {
	...
	game: for (const line of input) {
		const parts = line.split(": ");
		const game = Number(parts[0].split(" ")[1]);
		const hands = parts[1].split("; ");
		for (const hand of hands) {
			const parts2 = hand.split(", ");
			let red, green, blue;
			for (const part2 of parts2) {
				const redSplit = part2.split(" red");
				if (redSplit.length === 2) {
					red = redSplit[0];
					continue;
				}
				const greenSplit = part2.split(" green");
				if (greenSplit.length === 2) {
					green = greenSplit[0];
					continue;
				}
				const blueSplit = part2.split(" blue");
				if (blueSplit.length === 2) {
					blue = blueSplit[0];
					continue;
				}
			}
			if (red > actualCubes["red"] || green > actualCubes["green"] || blue > actualCubes["blue"]) continue game;
		}
		sum += game;
	};
	...
	game: for (const line of input) {
		...
		for (const hand of hands) {
			...
			for (const part2 of parts2) {
				const redSplit = part2.split(" red");
				if (redSplit.length === 2) {
					red = Number(redSplit[0]);
					continue;
				}
				const greenSplit = part2.split(" green");
				if (greenSplit.length === 2) {
					green = Number(greenSplit[0]);
					continue;
				}
				const blueSplit = part2.split(" blue");
				if (blueSplit.length === 2) {
					blue = Number(blueSplit[0]);
					continue;
				}
			}
			if (red > minCubes["red"]) {
			...
			if (green > minCubes["green"]) {
				minCubes["green"] = green;
			}
			if (blue > minCubes["blue"]) {
				minCubes["blue"] = blue;
			}
		}
